...
import { Search, Filter, ShieldAlert, Key, Globe, LogIn, Sparkles, ChevronRight, Info } from "lucide-react"
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <Input placeholder="Search logs..." className="pl-10 w-64 h-9" />
            </div>
          ...
          <CardContent className="p-0">
            <div className="divide-y">
              {LOGS.map((log) => {
                ...
                return (
                  <div key={log.id} className="p-4 md:p-6 hover:bg-secondary/10 transition-colors">
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                      <div className="flex items-start gap-4">
                        <div className={`p-2 rounded-lg ${log.severity === 'High' ? 'bg-destructive/10 text-destructive' : 'bg-primary/10 text-primary'}`}>
                          <Icon className="w-5 h-5" />
                        </div>
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-code text-xs font-bold uppercase tracking-tight text-muted-foreground">{log.event}</span>
                            <Badge variant={getSeverityColor(log.severity) as any} className="text-[10px] h-4">
                              {log.severity}
                            </Badge>
                      ...
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        className="gap-2 text-primary hover:text-primary hover:bg-primary/5 shrink-0 self-start"
                      ...
                      >
                        ...
                        {isClarified ? 'Clarified' : 'AI Clarify'}
                      </Button>
                    </div>

                    {isClarified && (
                      <div className="mt-6 ml-0 md:ml-12 p-6 rounded-2xl bg-primary/5 border border-primary/10 space-y-4 animate-in zoom-in-95 duration-300">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Explanation</p>
                            <p className="text-sm leading-relaxed">{isClarified.explanation}</p>
                          </div>
                          <div>
                            <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Security Implications</p>
                            <p className="text-sm leading-relaxed">{isClarified.implications}</p>
                          </div>
                        </div>
                        <div className="pt-2 border-t border-primary/10">
                          <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Recommendation</p>
                          <p className="text-sm font-medium text-primary-foreground bg-primary px-3 py-2 rounded-lg inline-block">
                            {isClarified.advice}
                          </p>
                        </div>
                      </div>
