...
import { Shield, Sparkles, XCircle, CheckCircle, Clock, Info, ExternalLink } from "lucide-react"
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 rounded-xl bg-secondary flex items-center justify-center overflow-hidden shrink-0">
                          <Image src={platformLogo || ""} alt={request.platform} width={48} height={48} />
                        </div>
                        <div className="space-y-1">
                          ...
                          {aiSummary[request.id] && (
                            <div className="mt-4 p-4 rounded-lg bg-primary/5 border border-primary/10 animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="flex flex-wrap gap-2 lg:flex-nowrap">
                        ...
                        {request.status === "Pending" && (
                          <>
                            <Button size="sm" className="gap-2 bg-green-600 hover:bg-green-700" onClick={() => handleAction(request.id, "Approved")}>
                              <CheckCircle className="w-4 h-4" /> Approve
                            </Button>
                            <Button size="sm" variant="destructive" className="gap-2" onClick={() => handleAction(request.id, "Denied")}>
                        ...
                        {request.status === "Approved" && (
                          <Button size="sm" variant="outline" className="text-destructive border-destructive hover:bg-destructive/5" onClick={() => handleAction(request.id, "Denied")}>
                            Revoke Access
                          </Button>
                        )}

                        {request.status === "Denied" && (
                          <Button size="sm" variant="outline" onClick={() => handleAction(request.id, "Approved")}>
                            Re-approve
                          </Button>
                        )}
                      </div>
